...
{
    common.writeLog(`${req.method} ${req.url}`);
    next();
});
...
// Host on / and /v388/ path
app.use('/', appRouter);
app.use('/v388/', appRouter);

app.all('*', (req, res) =>
{
    common.writeLog(`${req.method} ${req.url} is unhandled`);
    res.status(200).end();
})
...
https.createServer({ key, cert, minVersion: "TLSv1" }, muchaApp).listen(PORT_MUCHA, '0.0.0.0', 511, () =>
{
    console.log(`Mucha server listening on port ${PORT_MUCHA}!`);
})

// Create the game server
https.createServer({ key, cert, minVersion: "TLSv1"}, app).listen(PORT_BNGI, '0.0.0.0', 511, () =>
{
